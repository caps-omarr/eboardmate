...
const listHouses = computed(() => props.boardingHouses || []);
...
const fetchRealDistances = async () => {
    ...
    await Promise.all(listHouses.value.map(async (house) => {
    ...
    <PublicLayout>
        <section class="py-4 py-md-5 bg-body-tertiary min-vh-100">
            <div class="container px-3 px-md-4">
                ...
                <!-- Native Page Header with Top-Left Back Button -->
                <div class="mb-4">
                    <div class="mb-3">
                        <Link href="/" class="btn btn-sm border-secondary-subtle bg-body text-body-emphasis shadow-sm rounded-pill fw-semibold px-3 py-2 transition-all d-inline-flex align-items-center gap-1">
                            <i class="bi bi-arrow-left text-success fs-6"></i> Back to Home
                        </Link>
                    </div>
