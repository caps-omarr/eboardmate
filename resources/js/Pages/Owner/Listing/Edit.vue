    ...
    <OwnerLayout>
        ...
        <div class="container-fluid pb-5 px-0 px-md-3 max-w-desktop mx-auto">
            ...
            <section v-if="!boardingHouse" class="mx-3 mx-md-0 ebm-card p-4 p-md-5 text-center shadow-sm rounded-4">
                <i class="bi bi-house display-5 text-secondary opacity-50 mb-3 d-block"></i>
                ...
                <div class="row g-4">
                    <div class="col-lg-5">
                        <div class="ebm-card p-4 rounded-4 shadow-sm border border-secondary-subtle bg-body mb-4">
                            ...
                            <span class="placeholder col-6 py-2 rounded-2 d-block bg-secondary bg-opacity-25"></span>
                        </div>
                    </div>
                    <div class="col-lg-7">
                ...
                <div class="col-lg-5 p-0 pe-lg-3">
                    ...
                    <div class="bg-body p-4 rounded-4 shadow-sm mb-4 border border-secondary-subtle">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            ...
                            <span class="badge rounded-2 px-2.5 py-1 shadow-sm text-capitalize" :class="statusBadgeClass">
                                {{ boardingHouse.status }}
                            </span>
                        </div>

                        <div class="d-flex align-items-center gap-2 mb-4">
                            <span v-if="boardingHouse.is_verified" class="badge badge-soft-success rounded-2 px-2 py-1"><i class="bi bi-patch-check-fill me-1"></i> Verified</span>
                            <span v-else class="badge badge-soft-warning rounded-2 px-2 py-1"><i class="bi bi-exclamation-circle-fill me-1"></i> Not Verified</span>
                        </div>
                        ...
                        <div v-if="boardingHouse.rejection_reason" class="alert alert-danger mt-3 mb-0 rounded-3 border-0 small">
                            <strong>Rejection Reason:</strong> {{ boardingHouse.rejection_reason }}
                        </div>
                        <div v-if="boardingHouse.deactivated_reason" class="alert alert-secondary mt-3 mb-0 rounded-3 border-0 small">
                    ...
                    <div class="bg-body p-4 rounded-4 shadow-sm mb-4 border border-secondary-subtle">
                        ...
                        <form @submit.prevent="submitPhoto">
                            <div class="mb-3">
                                <label 
                                    for="listing-photo" 
                                    class="btn btn-outline-secondary w-100 py-3 rounded-3 fw-semibold d-flex flex-column align-items-center justify-content-center gap-1 border-dashed bg-body-tertiary position-relative z-2 cursor-pointer"
                                ...
                                <div v-if="photoPreview" class="mt-3 text-center p-2 bg-body-tertiary rounded-3 border border-secondary-subtle overflow-hidden">
                            ...
                            <div class="mb-4">
                                <input id="alt_text" v-model="photoForm.alt_text" type="text" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle" style="min-height: 44px;" :class="{ 'is-invalid': photoForm.errors.alt_text }" placeholder="Alt Text (e.g. Front View)">
                                <div v-if="photoForm.errors.alt_text" class="invalid-feedback fw-bold ps-2">{{ photoForm.errors.alt_text }}</div>
                            </div>

                            <button type="submit" class="btn btn-success rounded-3 w-100 fw-semibold shadow-sm py-2 d-flex align-items-center justify-content-center gap-2" style="min-height: 44px;" :disabled="photoForm.processing || !photoForm.photo">
                                <span v-if="photoForm.processing"><span class="spinner-border spinner-border-sm me-2"></span>Uploading...</span>
                                <span v-else>Upload Selected Photo</span>
                            </button>
                        </form>
                    </div>
                    ...
                    <div class="bg-body p-4 rounded-4 shadow-sm mb-4 mb-lg-0 border border-secondary-subtle">
                        <div class="d-flex justify-content-between align-items-end mb-3">
                            ...
                            <span class="badge bg-body-tertiary text-body-secondary border border-secondary-subtle rounded-2 px-2 py-1">{{ boardingHouse.photos?.length || 0 }} Photos</span>
                        </div>
                        ...
                        <div v-if="boardingHouse.photos && boardingHouse.photos.length" class="native-gallery-scroll d-flex gap-3 overflow-x-auto pb-3 hide-scrollbar">
                            ...
                            <div v-for="photo in boardingHouse.photos" :key="photo.id" class="gallery-card flex-shrink-0 position-relative rounded-3 overflow-hidden border border-secondary-subtle bg-body-tertiary">
                                
                                <!-- Primary Badge -->
                                <span v-if="photo.is_primary" class="badge badge-soft-success position-absolute top-0 start-0 m-2 rounded-2 px-2 py-1 shadow-sm" style="z-index: 10;">Primary</span>
                                ...
                                <div class="p-2 d-flex gap-2">
                                    <button v-if="!photo.is_primary" type="button" class="btn btn-sm btn-light border-secondary-subtle flex-grow-1 rounded-2" style="font-size: 0.75rem; font-weight: 600;" :disabled="primaryForm.processing" @click="setPrimaryPhoto(photo)">
                                        Set Primary
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-danger flex-grow-1 rounded-2 d-flex align-items-center justify-content-center gap-1" :disabled="deleteForm.processing" @click="deletePhoto(photo)" style="font-size: 0.75rem; font-weight: 600;">
                        ...
                        <div v-else class="text-center p-4 bg-body-tertiary rounded-3 border border-secondary-subtle">
                            <i class="bi bi-images display-5 text-secondary opacity-50 mb-2 d-block"></i>
                            <h3 class="h6 fw-bold mb-1">No photos yet</h3>
                            <p class="text-body-secondary small mb-0">Upload a photo above to display on your listing.</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-body p-4 p-md-5 rounded-4 shadow-sm border border-secondary-subtle">
                        ...
                        <form @submit.prevent="submitListing">
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-12">
                                    <label for="address" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Full Address</label>
                                    <input id="address" v-model="form.address" type="text" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.address }" placeholder="Talibon, Bohol">
                                    <div v-if="form.errors.address" class="invalid-feedback fw-bold ps-2">{{ form.errors.address }}</div>
                                </div>

                                <!-- 🗺️ Explicit GPS Coordinates -->
                                <div class="col-12 col-md-6">
                                    <label for="latitude" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">
                                    ...
                                    <input 
                                        ...
                                        min="-90" 
                                        max="90" 
                                        class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2 font-monospace" 
                                ...
                                <div class="col-12 col-md-6">
                                    <label for="longitude" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">
                                ...
                                <div class="col-12">
                                    <label for="rent_price" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Monthly Rent (₱)</label>
                                    <input id="rent_price" v-model="form.rent_price" type="number" step="0.01" min="0" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2 text-success fw-bold" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.rent_price }">
                            ...
                            <!-- Descriptions -->
                            <div class="mb-4">
                                <label for="description" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Description</label>
                                <textarea id="description" v-model="form.description" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2" :class="{ 'is-invalid': form.errors.description }" rows="4" placeholder="Describe your boarding house..."></textarea>
                                <div v-if="form.errors.description" class="invalid-feedback fw-bold ps-2">{{ form.errors.description }}</div>
                            </div>

                            <div class="mb-4">
                                <label for="location_description" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Location Landmarks</label>
                                <textarea id="location_description" v-model="form.location_description" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2" :class="{ 'is-invalid': form.errors.location_description }" rows="2" placeholder="e.g., near TPC gate..."></textarea>
                            ...
                            <div class="bg-body-tertiary p-3 rounded-4 border border-secondary-subtle mb-4">
                                <div class="row g-3">
                                    <div class="col-6">
                                        <label for="total_rooms" class="form-label fw-bold small text-body-secondary mb-2">Total Rooms</label>
                                        <input id="total_rooms" v-model="form.total_rooms" type="number" min="0" class="form-control rounded-3 border-secondary-subtle text-center" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.total_rooms }">
                                    </div>
                                    <div class="col-6">
                                        <label for="available_rooms" class="form-label fw-bold small text-body-secondary mb-2">Available Rooms</label>
                                        <input id="available_rooms" v-model="form.available_rooms" type="number" min="0" class="form-control rounded-3 border-secondary-subtle text-center" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.available_rooms }">
                                    </div>
                                    <div class="col-6">
                                        <label for="total_bedspaces" class="form-label fw-bold small text-body-secondary mb-2">Total Beds</label>
                                        <input id="total_bedspaces" v-model="form.total_bedspaces" type="number" min="0" class="form-control rounded-3 border-secondary-subtle text-center" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.total_bedspaces }">
                                    </div>
                                    <div class="col-6">
                                        <label for="available_bedspaces" class="form-label fw-bold small text-body-secondary mb-2">Available Beds</label>
                                        <input id="available_bedspaces" v-model="form.available_bedspaces" type="number" min="0" class="form-control rounded-3 border-secondary-subtle text-center" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.available_bedspaces }">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                <label for="allowed_genders" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Gender Restriction</label>
                                <select id="allowed_genders" v-model="form.allowed_genders" class="form-select bg-body-tertiary rounded-3 border-secondary-subtle py-2" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.allowed_genders }">
                                ...
                                <div class="row g-3">
                                    <div class="col-12 col-md-6">
                                        <div class="form-check form-switch mb-1">
                                            ...
                                            <label for="includes_water" class="form-check-label fw-medium"><i class="bi bi-droplet text-primary me-1"></i> Water Included</label>
                                        </div>
                                        <div v-if="!form.includes_water" class="mt-2">
                                            <select v-model="form.water_billing_details" class="form-select form-select-sm bg-body text-body-emphasis border-secondary-subtle rounded-3" style="min-height: 38px;">
                                    ...
                                    <div class="col-12 col-md-6">
                                        <div class="form-check form-switch mb-1">
                                            ...
                                            <label for="includes_electricity" class="form-check-label fw-medium"><i class="bi bi-lightning-charge text-warning me-1"></i> Electricity Included</label>
                                        </div>
                                        <div v-if="!form.includes_electricity" class="mt-2">
                                            <select v-model="form.electricity_billing_details" class="form-select form-select-sm bg-body text-body-emphasis border-secondary-subtle rounded-3" style="min-height: 38px;">
                            ...
                            <!-- Amenities & Rules -->
                            <div class="mb-4">
                                <label for="amenities_text" class="form-label fw-bold small text-body-secondary text-uppercase mb-2">Amenities (Comma separated)</label>
                                <input id="amenities_text" v-model="form.amenities_text" type="text" class="form-control bg-body-tertiary rounded-3 border-secondary-subtle py-2" style="min-height: 44px;" :class="{ 'is-invalid': form.errors.amenities_text }" placeholder="WiFi, Water, Study Area">
                            ...
                            <button type="submit" class="btn btn-success rounded-3 w-100 fw-semibold shadow-sm py-2.5 mt-2 d-flex align-items-center justify-content-center gap-2" style="min-height: 48px;" :disabled="form.processing">
                                ...
                                <span v-else>Update Property Details</span>
                            </button>
                        </form>
                    </div>
                </div>
